// data/consular-offices.ts — 真实领区数据（使领馆/签证中心 → 覆盖省份）
...
// 本模块只承载数据；查询逻辑（getConsularOffices）与开发期校验（validateDistrictCoverage）在 consular-districts.ts。
...
export const DEFAULT_OFFICES: Record<string, ConsularOffice[]> = {
  // ═══ 日本：按户籍划分，经指定代办机构递交 ═══
  japan: [
